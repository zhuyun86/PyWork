...
def calculateRadiomicsFromJson(niiFile, jsonFile,wavelet,LoG,Square,SquareRoot,Logarithm,\
    ...
    for prefix, _, value in p:
        ...
        #     nodules[label]['sliceMask'] = ''
        if re.match('Nodules.item\d+.VerifiedNodule.mask.xBegin', prefix) or prefix == 'Nodules.item.mask.xBegin':
            nodules[label]['xBegin'] = float(value)
        if re.match('Nodules.item\d+.VerifiedNodule.mask.yBegin', prefix) or prefix == 'Nodules.item.mask.yBegin':
            nodules[label]['yBegin'] = float(value)
        if re.match('Nodules.item\d+.VerifiedNodule.mask.zBegin', prefix) or prefix == 'Nodules.item.mask.zBegin':
            nodules[label]['zBegin'] = float(value)
        if re.match('Nodules.item\d+.DimInPixelX', prefix) or prefix == 'Nodules.item.DimInPixelX':
            nodules[label]['DimInPixelX'] =int(float(value))-1
        if re.match('Nodules.item\d+.DimInPixelY', prefix) or prefix == 'Nodules.item.DimInPixelY':
            nodules[label]['DimInPixelY'] = int(float(value))-1
        if re.match('Nodules.item\d+.DimInPixelZ', prefix) or prefix == 'Nodules.item.DimInPixelZ':
            nodules[label]['DimInPixelZ'] = int(float(value))-1
        if re.match('Nodules.item\d+.VerifiedNodule.mask.sliceMask.item', prefix) or prefix == 'Nodules.item.mask.sliceMask.item':
...
if __name__ == '__main__':
    filePath = r"F:\GYFY_nii\201601-03_nii"
    ...
    for subfile in niiFileList:
        ...
        args.json = args.nii.replace(".nii","_CAD_Lung_yxg.json")
